@echo off
REM =========================================================
REM DB 파트 전체 데이터 수집을 순서대로 한 번에 실행
REM 사용법: DB 폴더 안에서 run_all.bat 실행
REM 사전 준비: .env 파일에 필요한 값 채워두기, Postgres 컨테이너 켜져 있어야 함
REM =========================================================

echo ===== 0. 스키마 생성 (테이블이 없을 때만 필요, 이미 있으면 생략 가능) =====
REM docker exec -i startup_db psql -U admin -d startup_platform < schema.sql
echo   (필요 시 위 줄 주석 해제 후 실행 = 'REM' 이 단어 지우면 됨.)
